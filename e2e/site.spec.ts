...
for (const path of routes) {
  test(`${path} renders accessibly on desktop and mobile`, async ({ page }) => {
    ...
    for (const width of [390, 320]) {
      ...
      expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), `overflow on ${path} at ${width}px`).toBe(true);
      await expect(page.locator('main h1')).toBeVisible();
    }
...
test('mobile navigation supports Escape, links, and desktop resize', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Open navigation' }).click();
  await page.getByRole('navigation', { name: 'Mobile navigation' }).getByRole('link', { name: 'About', exact: false }).click();
...
test('star cloud animates, pauses, persists, and respects reduced motion', async ({ page }) => {
  ...
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await expect(page.getByRole('button', { name: 'Animations reduced by system preference' })).toBeDisabled();
...
test('direct navigation, back, refresh, and unknown routes work', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'This page took a different path.' })).toBeVisible();
