...
it('provides a pause control contract and reduced-motion override for decorative artwork', () => {
  expect(capabilitiesStyles).toMatch(/\.cap-concept-art \.art-elements\s*\{[^}]*animation:\s*concept-drift/);
  expect(capabilitiesStyles).toMatch(/html\[data-motion="paused"\] \.cap-concept-art \.art-elements\s*\{[^}]*animation-play-state:\s*paused/);
  expect(capabilitiesStyles).toMatch(/@media\s*\(prefers-reduced-motion:\s*reduce\)\s*\{\s*\.cap-concept-art \.art-elements\s*\{[^}]*animation:\s*none/);
});
...
describe('Careers', () => {
  it('invites talent-network introductions by real email without inventing vacancies or benefits', () => {
    ...
    expect(screen.getByRole('heading', { name: 'A conversation, not a job posting.' })).toBeInTheDocument();
    ...
    expect(introduction).toHaveAttribute('href', 'mailto:contact@hightech.fit?subject=Talent%20network%20introduction');
    expect(screen.getByRole('link', { name: /Meet the studio/ })).toHaveAttribute('href', '/about');
    expect(document.body.textContent).not.toMatch(/Apply Now|We're Hiring|\$5,000|Unlimited PTO|Equity Package|world-class|Senior ML Engineer/);
...
describe('Projects', () => {
  ...
  it('clears expanded details when changing filters', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Explore Fieldnote' })).toHaveAttribute('aria-expanded', 'false');
  });

  it.each(['Fieldnote', 'Shelf', 'Trace', 'Openbook', 'Shift', 'Wayline'])('opens and closes %s by keyboard with explicit validation boundaries', async (title) => {
    const user = userEvent.setup();
    render(<MemoryRouter><ProjectsPage /></MemoryRouter>);
      ...
      expect(within(panel!).getByRole('heading', { name: 'Before a real build' })).toBeInTheDocument();
      expect(within(panel!).getByRole('link', { name: /Discuss a similar idea/ })).toHaveAttribute('href', '/contact');
  ...
  it('filters concepts by delivery type and announces the current result count', async () => {
    ...
    expect(within(filters).getByRole('button', { name: 'All concepts' })).toHaveAttribute('aria-pressed', 'true');
  });
  ...
  it('labels every project as a concept rather than presenting invented client work', () => {
    ...
    expect(document.body.textContent).not.toMatch(/MediVision|GlobalRetail|SecureBank|Projects Delivered|Client Satisfaction|99\.2%|\$100M|12 engineers/);
  });
});
...
describe('Industries', () => {
  it('lets keyboard users inspect six possible application areas without outcome claims', async () => {
    const user = userEvent.setup();
    render(<MemoryRouter><IndustriesPage /></MemoryRouter>);
    ...
    expect(screen.getByText(/application areas, not a list of client engagements/i)).toBeInTheDocument();
...
describe('Services', () => {
  it('offers four clearly scoped delivery disciplines with a real project enquiry link', () => {
    ...
    for (const name of ['Applied AI', 'Web applications', 'Mobile experiences', 'Product delivery']) {
  ...
  it('explains delivery as an ordered process with a tangible handover', () => {
    ...
    expect(within(process).getAllByRole('heading').map(heading => heading.textContent))
      .toEqual(['Frame the problem', 'Make it tangible', 'Build in the open', 'Launch & hand over']);
    expect(within(process).getByText(/source code, documentation, and a plan/i)).toBeInTheDocument();
  });
});
