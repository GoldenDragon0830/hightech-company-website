...
export default function ContactForm({ recipient }: { recipient: string }) {
    ...
    return {
      ...
      'aria-describedby':
        [
          ...
          errors[field] ? `contact-${field}-error` : '',
        ]
          .filter(Boolean)
          .join(' ') || undefined,
    };
  ...
  return (
    <div className="contact-form-panel">
      ...
      <p id="contact-draft-help">
        This form prepares a draft. You still need to press Send in your email app.
      </p>
      {draft ? (
        <section className="contact-ready" aria-labelledby="contact-ready-heading">
          <h3 ref={readyRef} tabIndex={-1} id="contact-ready-heading">
            Your draft is ready
          </h3>
          <p>Nothing has been sent. Review your draft and press Send in your email app.</p>
          {draft.needsManualPaste && (
            <p className="contact-copy-fallback">
              This brief is too long for a reliable email link. Copy the briefing below, open your
              email app, then paste it before sending. The link will fill only the recipient and
              subject.
            </p>
          )}
          <div className="contact-ready-actions">
            <a className="button button-primary" href={draft.href}>
              Open email app
            </a>
            <button className="button button-outline" type="button" onClick={copyBriefing}>
              Copy briefing
            </button>
          </div>
          <p>
            No email app configured? Copy the briefing and paste it into a new message in your
          ...
          {copyFailed && (
            <div className="contact-copy-fallback">
              <p role="alert">
                Automatic copy is unavailable. Select the briefing below, then use your device’s
                Copy command. Paste it into your email app and press Send.
              </p>
              <button
                type="button"
                className="text-link"
        ...
        >
          {Object.keys(errors).length > 0 && (
            <p role="alert" className="contact-error-summary">
              Check the highlighted fields before preparing your draft.
            </p>
          )}
          <div className="contact-field">
          ...
          <div className="contact-field">
            ...
            {errors.topic && (
              <p id="contact-topic-error" className="contact-error">
                {errors.topic[0]}
              </p>
            )}
          </div>
          ...
          <div className="contact-field">
            ...
            <select
              ...
              name="budget"
              value={brief.budget}
              onChange={(event) => setBrief({ ...brief, budget: event.target.value })}
            >
          ...
          <div className="contact-field contact-field-wide">
            ...
            />
            {errors.company && (
              <p id="contact-company-error" className="contact-error">
                {errors.company[0]}
              </p>
            )}
          </div>
          <div className="contact-field contact-field-wide">
            ...
            <p id="contact-message-help" className="contact-field-help">
              Your goal, who it is for, and any timing or technical constraints. 20 characters
              minimum. 1,200 characters maximum.{' '}
              <span className="contact-count">
                {brief.message.length} / {MAX_BRIEF_LENGTH}
              </span>
            </p>
            {errors.message && (
      ...
      <div className="contact-privacy" id="contact-privacy">
        <p>
          <strong>Your details, your choice.</strong> This form does not send or save your details
          to a server. The brief is held in this page’s memory. Copy anything you want to keep
          before leaving.
        </p>
        <p>
          Opening your email app passes the draft to that app. Copying puts the briefing on your
          device’s clipboard. Your email provider handles the message when you send it.
        </p>
