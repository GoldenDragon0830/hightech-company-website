...
export default function TeamCard({ member }: TeamCardProps) {
  ...
  return (
    <article className="team-card" aria-labelledby={nameId}>
      <div className="team-card__portrait">
        <img
          src={member.avatar}
          alt={`Abstract illustrated avatar for ${member.name} (sample profile)`}
          width={640}
          height={480}
          loading="lazy"
          decoding="async"
        />
        <span className="team-card__sample">Sample profile</span>
      </div>
      <div className="team-card__content">
        ...
        <ul className="tags team-card__skills" aria-label={`${member.name}'s skills`}>
          {member.skills.map((skill) => <li className="tag" key={skill}>{skill}</li>)}
        </ul>
        {socials.length > 0 && (
