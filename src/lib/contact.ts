...
export const contactTopics = ['Web application', 'Mobile application', 'AI & automation', 'Technical discovery', 'Something else'] as const;
export const contactBudgets = ['Not sure yet', 'Under $10,000', '$10,000–$25,000', '$25,000–$50,000', '$50,000+'] as const;
...
export const contactSchema = z.object({
  ...
  email: z.string().trim().email('Enter a valid email address.').max(254, 'Keep your email address to 254 characters.'),
  topic: z.enum(contactTopics, { errorMap: () => ({ message: 'Choose a topic.' }) }),
  message: z.string().trim().min(20, 'Tell us a little about your project (at least 20 characters).').max(MAX_BRIEF_LENGTH, 'Keep your brief to 1,200 characters.'),
  budget: z.enum(['', ...contactBudgets], { errorMap: () => ({ message: 'Choose a budget option or leave it blank.' }) }),
